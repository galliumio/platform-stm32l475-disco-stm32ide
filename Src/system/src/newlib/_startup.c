...
_start (void)
{
  // Gallium - On STM32L476-NUCLEO the stack pointer is not set correctly
  //           when using openocd to load the program. It is therefore
  //           set here explicitly. Note that the debugger starts at _start
  //           rather than Reset_Handler().
  //           Must reserve stack frame for calls made in this function.
  asm volatile
  (
      " ldr    sp,=_estack \n"
      " sub    sp, #16 \n"
